 ...
 float elasticity, int num_particles, int num_springs)
{
  ...
  for (int i=0; i<num_springs; i++) {
    ...
    const float rx_last = pos_last[p1*3] - pos_last[p0*3];
    const float ry_last = pos_last[p1*3+1] - pos_last[p0*3+1];
    const float rz_last = pos_last[p1*3+2] - pos_last[p0*3+2];
    const float d = sqrtf(rx*rx + ry*ry + rz*rz);
    const float d_last = sqrtf(rx_last*rx_last + ry_last*ry_last + rz_last*rz_last);
    rx /= d; ry /= d; rz /= d;
    const float displacement = 
      elasticity*(d - natural_length[i]);
    const float displace_x = displacement*rx_last;
    const float displace_y = displacement*ry_last;
    const float displace_z = displacement*rz_last;
...
int main()
{
  ...
  InitWindow(WINDOW_W, WINDOW_H, "wtf");
  int should_quit = 0;
  while (!should_quit) {
    ...
    for (int i=0; i<8; i++) {
      update_system(pos, pos_last, inv_mass, point0, point1, natural_len, 1.0f, PARTICLE_NUM, spring_num);
    }
    cam.target = average_pos(pos, PARTICLE_NUM);
    BeginDrawing();
    ClearBackground(BLACK);
    BeginMode3D(cam);
    DrawLine3D((Vector3){-4.0f,0.0f,0.0f}, (Vector3){4.0f,0.0f,0.0f},DARKGRAY);
    DrawSphere(*((Vector3 *)(pos+54*3)), 0.15f, RED);
